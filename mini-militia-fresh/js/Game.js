...
this.ground=newTileGround(this.ctx);
this.collisionHandler=newCollisionHandler(this.ctx,this.camera,this.ground.mapArray);
this.gameBullets=[];
this.shyame=newShyame(this.ctx,{x:600,y:200},this.canvas,this.camera,this.collisionHandler,this.resources);
this.camera=newCamera(this.ctx,this.shyame);
this.robotUnits={};

this.introAudio=this.resources.getAudio('intro');
this.introAudio.play();

varrandomNoOfRobotUnits=6;
...
varoffsetY=100;
vardirectionCount=0;
for(vari=0;i<Object.size(_this.robotUnits);i++){
...
vardistX=_this.robotUnits[i].position.x-_this.shyame.actor.position.x;
vardistY=_this.robotUnits[i].position.y-_this.shyame.actor.position.y;
varcOffset=200;
vardistance=Util.calculateDistance(_this.robotUnits[i].position.x,_this.robotUnits[i].position.y,_this.shyame.actor.position.x,_this.shyame.actor.position.y);
...
vartBaseX=_this.shyame.actor.position.x+vectorX*cOffset;
vartBaseY=_this.shyame.actor.position.y+vectorY*cOffset;
vartDistance=Util.calculateDistance(_this.robotUnits[i].position.x,_this.robotUnits[i].position.y,tBaseX,tBaseY);

vartDistX=_this.robotUnits[i].position.x-tBaseX;
vartDistY=_this.robotUnits[i].position.y-tBaseY;
vartVectorX=tDistX/tDistance;
vartVectorY=tDistY/tDistance;
vartOffset=20;

varxPerp=tOffset*tVectorX;
varyPerp=tOffset*tVectorY;
...
varrx=Util.getRandomInt(0,_this.canvas.width/3);
varry=Util.getRandomInt(-_this.canvas.height,0);
varrobotUnit=newRobotUnit(Object.size(_this.robotUnits),_this.ctx,{x:rx,y:ry},_this.shyame,_this.robotUnits,_this.ground.mapArray,_this.collisionHandler,_this,_this.resources);
...
}elseif(!_this.gameBullets[i].hit){
...
varrefreshedBullets={};
varrefreshedBulletsCount=0;
for(vari=0;i<Object.size(_this.gameBullets);i++){
if(_this.gameBullets[i].actorType=='shyame'){
varrefreshedRobotUnitsCount=0;
varrefreshedRobotUnits={};
for(varj=0;j<Object.size(_this.robotUnits);j++){
...
}elseif(_this.gameBullets[i].actorType=='robot-unit'){
...
}elseif(_this.shyame.actor.health>0){
...
//returnstrueiftheactorandbulletarecolliding
this.rectCircleColliding=function(bullet,actor){
vardistX=Math.abs(bullet.toPosition.x-actor.position.x-actor.actorWidth/2);
vardistY=Math.abs(bullet.toPosition.y-actor.position.y-actor.actorHeight/2);
...
vardx=distX-actor.actorWidth/2;
vardy=distY-actor.actorHeight/2;
